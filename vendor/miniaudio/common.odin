...
when MINIAUDIO_SHARED {
	#panic("Shared linking for miniaudio is not supported yet", #trigger_location)
}
...
when !#exists(LIB) {
	...
	#panic("Could not find the compiled miniaudio library, it can be compiled by running `make -C \"" + ODIN_ROOT + "vendor/miniaudio/src\"`", #trigger_location)
}
